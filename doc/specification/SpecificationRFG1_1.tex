...
\newpage
\subsection{Register}
Registers are the smallest addressable units in a registerfile. A register can consist of one ore more fields with different widths and attributes. These variants of generated hardware depending on the attributes is described in this subsection. 
...
With the sticky feature a field is generated in which the bits in the field can only be set from the hardware. The field can only be resetted from the software interface or with a reset.\\
\\
RFG Description:
...
The diffirence in the verilog output with the sticky attribute is that the new hardware value is ored on write with the register value itself line 36 to line 37.
\newpage
